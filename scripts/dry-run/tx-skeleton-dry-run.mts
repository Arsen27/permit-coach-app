...
// the server's content tree holds tx-class-c 3.3.0 (seeded, on both channels)
// and 2.0.0 (built by scripts/build-state-course.mjs). import_content_tree
// registers 2.0.0 and merges its questions into the working bank;
...
const { db } = await createContentDb({ courses: ['tx-class-c'], versions: ['1.0.0'], channels: true });
...
show('channel_status before', await call('channel_status', { course: 'tx-class-c' }));
show('import_content_tree 2.0.0', await call('import_content_tree', { course: 'tx-class-c', versions: ['2.0.0'] }), 900);
show('publish_staging 2.0.0', await call('publish_staging', { course: 'tx-class-c', version: '2.0.0', reason: 'skeleton dry run' }));
show('publish_bank_staging', await call('publish_bank_staging', { course: 'tx-class-c' }));
show('channel_status after', await call('channel_status', { course: 'tx-class-c' }));
const outline = show('get_outline latest', await call('get_outline', { course: 'tx-class-c' }), 400);
console.log('modules', outline.data?.modules?.length, 'lessons', outline.data?.modules?.reduce((n: number, m: any) => n + m.lessons.length, 0));
show('read_lesson tx-speed-and-space', await call('read_lesson', { course: 'tx-class-c', lesson: 'tx-speed-and-space' }), 1200);
...
const bank = await get('/v1/bank/tx-class-c/doc?channel=staging');
const bankIds = new Set((bank.body?.questions ?? []).map((q: any) => q.questionId));
const out = await get('/v1/course/tx-class-c/outline?channel=staging');
console.log('\nstaging outline status', out.status, 'version', out.body?.deliveryVersion ?? out.body?.version);
const lessonDoc = await get('/v1/course/tx-class-c/lesson/tx-speed-and-space?channel=staging');
